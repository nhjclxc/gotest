...
import (
	"context"
	"go_base_project/application"
	"go_base_project/config"
	"go_base_project/cron"
	crontasks2 "go_base_project/cron/crontasks"
...
var rootCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		// 3.1 添加云端API任务
		cloudAPITask := crontasks2.NewCloudAPITask(
			serviceManager.GetResource(),
			serviceManager.GetEventBus(),
		)
		cronTaskManager.AddTask(cloudAPITask)
